    ...
    <span>
        <template v-if="!template">
            <button v-if="!leitura"
                    ...
                    @click="selecionar()"><i class="fas fa-upload"></i> {{ label }}</button>
        </template>
        ...
        <div class="table-responsive" v-show="lista.length" v-if="!simples">
            <table class="table table-bordered table-hover table-condensed">
                ...
                <draggable v-model="lista" tag="tbody" :handle="{draggable:'.linha',handle:'.mover'}">
                        ...
                        <td class="text-center">
                            ...
                            <span v-show="!arquivo.imagem && ( !arquivo.chave || (arquivo.enviado && !arquivo.falhou))">
                                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
                                     ...
                                     viewBox="0 0 512 512" style="enable-background:new 0 0 512 512; height: 45px"
                                     xml:space="preserve"
...
export default {
    ...
    props: {
        ...
        template: { // passar uma marcação html nova como botao
            type: Boolean,
            required: false,
            default: false
        }
    ...
    methods: {
        ...
        remover: function (index) {
            ...
            if (this.lista[index].id && this.lista[index].temporario) {
                ...
                axios.post(this.lista[index].urlDelete, dados)
                    .then((response) => {
                        // console.log('apagou');
                    })
                    .catch((error) => {
            ...
            if (fileList.length > 0) {
                ...
                Array.from(fileList).forEach((file, index) => {
                    ...
                    if (arquivo.bytes > this.bytesLimite) {
                        this.mostraErro({}, "O arquivo \"" + arquivo.nome + "\" (" + arquivo.bytes + " bytes) deve ter um tamanho menor que " + this.bytesLimite + " bytes");
                        return true;//proximo loop
                    }
        ...
        enviarArquivo() {
            ...
            axios.post(this.url, dados, config)
                .then((response) => {
