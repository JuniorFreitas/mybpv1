<template>
    <div>
        <!--Janela de filtrar-->
        <modal id="janelaFiltrarLancamentos" ref="janelaFiltrarLancamentos" :modal-pai="modalPai" size="g"
               titulo="Filtrar lançamentos">
            <template slot="conteudo">
                <p>
                    Filtre os lançamentos por planos de conta dentro do período
                </p>
                    ...
                    <table class="tabela">
                        ...
                        <tr>
                            <th class="text-center">
                                <div class="form-group form-check">
                                    <input v-model="abaMovimentacoes.tudoMarcado" class="form-check-input"
                                           type="checkbox"
                        ...
                        <tr v-for="(obj,index) in abaMovimentacoes.listaPlanosFiltro" :key="obj.id">
                            <td class="text-center">
                                <div class="form-group form-check">
                                    <input v-model="abaMovimentacoes.filtrarPor" :value="obj.id"
                                           class="form-check-input"
                                           type="checkbox" @change="verificarCheckFiltrarTudo">
                                </div>
                            </td>
                            <td>
                                {{ obj.descricao }}
            ...
            <template slot="rodape">
                <button :disabled="abaMovimentacoes.listaPlanosFiltro.length=== 0" class="btn btn-sm btn-success"
                        type="button"
        ...
        <modal id="janelaApagarLancamento" :fechar="!formApagar.preload" :modal-pai="modalPai"
               titulo="Excluir lançamento">
            <template slot="conteudo">

                        <span v-show="formApagar.preload">
                            <i class="fa fa-spinner fa-pulse"></i> Apagando lançamento...
                        </span>
        ...
        <modal id="janelaLancamento" :fechar="!formLancamento.preload" :modal-pai="modalPai"
               :size="90" :titulo="formLancamento.titulo">
            <template slot="conteudo">
                <span v-show="formLancamento.preload">
                    <span v-show="formLancamento.editando">
                        <i class="fa fa-spinner fa-pulse"></i> Alterando lançamento...
                    </span>
                    <span v-show="!formLancamento.editando">
                        <i class="fa fa-spinner fa-pulse"></i> Cadastrando lançamento...
                    </span>
                </span>
                <form
                    v-if="!formLancamento.preload && !formLancamento.erro && (!formLancamento.cadastrado && !formLancamento.atualizado) && cliente">
                    ...
                    <div v-if="formLancamento.updated_at" class="form-group row">
                        ...
                        <div v-if="formLancamento.quem_cadastrou" class="col-sm-10">
                            <input :value="`Em ${formLancamento.created_at} por ${formLancamento.quem_cadastrou.nome}`"
                                   class="form-control-plaintext" readonly
                                   type="text">
                        </div>

                        <label v-show="formLancamento.created_at !== formLancamento.updated_at"
                               class="col-sm-2 col-form-label">Última
                            alteração:</label>
                        <div
                            v-if="formLancamento.quem_alterou && (formLancamento.created_at !== formLancamento.updated_at)"
                            class="col-sm-10">
                            <input :value="`Em ${formLancamento.updated_at} por ${formLancamento.quem_alterou.nome}`"
                                   class="form-control-plaintext" readonly
                                   type="text">
                        </div>
                    ...
                    <div class="row">
                        <div class="col-12 mb-3">Nome do Cliente: <strong>{{ cliente.razao_social }}</strong></div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <autocomplete :formsm="false"
                                                          v-model="formLancamento.plano_conta.descricao" :rows="20"
                                                          ...
                                                          @onselect="selecionaPlano"></autocomplete>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label>Tipo</label>
                                            <select v-model="formLancamento.operacao"
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <textarea v-model="formLancamento.descricao" class="form-control"
                                                      rows="3"></textarea>
                                        </div>
                                    </div>
                    ...
                    <div class="form-group col-md-6">
                        <label>Valor</label>
                        <h3>R$ {{ valorTotalLancamento | dinheiro }}</h3>
                    </div>
                    <!-- formas de pagamento-->
                    <fieldset>
                                ...
                                <tr v-for="(obj,index) in formLancamento.formas">
                                    <td>
                                        <select v-model="obj.forma_pagamento_id" class="form-control">
                                            ...
                                            <option v-for="forma in cliente.formas_pagamento" :disabled="!forma.ativo"
                                                    :value="forma.id">
                                                {{ forma.descricao }}
                                            </option>
                                        </select>
                                    ...
                                    <td>
                                        <input v-model="obj.valorFormat" v-mascara:dinheiroPN class="form-control"
                                               type="text">
                                    </td>
                                    <td>
                    ...
                    <fieldset>
                        ...
                        <div class="form-check mb-2">
                            <input id="agendarLancamento" v-model="formLancamento.agendar" class="form-check-input"
                                   type="checkbox">
                            <label class="form-check-label" for="agendarLancamento">Agendar
                                recebimento/pagamento</label>
                        </div>
                        ...
                        <div v-show="formLancamento.agendar && pode_realizar" class="row">
                            <div class="col-md-2">
                                <div class="form-check form-switch form-switch-lg mb-3" dir="ltr">
                                    <input id="checkboxConcluido" v-model="formLancamento.concluido"
                        ...
                        <div v-show="formLancamento.agendar && formLancamento.concluido && pode_realizar" class="row">
                            <div class="col-md-2">
                                <div class="form-group">
                                    <datepicker v-model="formLancamento.data_hora_concluido" :hora="true"
                                                posicao="up"></datepicker>
                                </div>
                            </div>
                        </div>
                ...
                <div v-show="!formLancamento.preload && formLancamento.erro" class="alert alert-danger"
                     role="alert">
                    <h4 class="text-center">
                        <i class="fas fa-times-circle"></i><br>
                        {{ formLancamento.msg }}
            ...
            <template slot="rodape">
                <button
                    v-show="!formLancamento.preload && !formLancamento.erro && (!formLancamento.cadastrado && !formLancamento.atualizado) && !formLancamento.editando"
                    class="btn btn-sm btn-primary" type="button"
                    @click="salvarLancamento">
                    <i class="fas fa-plus"></i> Inserir lançamento
                </button>
                <button
                    v-if="!formLancamento.preload && !formLancamento.erro && (!formLancamento.cadastrado && !formLancamento.atualizado) && formLancamento.editando"
                    class="btn btn-sm btn-primary" type="button"
                    @click="salvarLancamento">
        ...
        <p v-show="preload && !cliente" class="text-center">
            <i class="fa fa-spinner fa-pulse"></i> Buscando dados do cliente...
        </p>
        ...
        <div v-if="!preload && cliente">
            ...
            <ul class="nav nav-tabs bg-light mt-3" role="tablist" style="border-bottom: 1px solid #653232">
                <li class="nav-item">
                    <a id="aba-movimentacoes-tab" aria-controls="aba-movimentacoes" aria-selected="true"
                       class="nav-link active" data-toggle="pill"
                       href="#aba-movimentacoes" role="tab">Lançamentos</a>
                </li>
            ...
            <div class="tab-content mb-3">
                ...
                <!-- Aba movimentacoes -->
                <div id="aba-movimentacoes" aria-labelledby="aba-movimentacoes-tab" class="tab-pane fade show active"
                     role="tabpanel">
                    <div class="col mt-2 mb-2">
                        ...
                        <div class="form-row">
                            <div class="col-12 col-md-4">
                                <div class="form-check mb-2">
                                    <input id="por_periodo_lancamento" v-model="controle.dados.por_periodo"
                                           class="form-check-input" type="checkbox" @change="atualizaContaCorrente">
                                    <label class="form-check-label" for="por_periodo_lancamento">Por período</label>
                                </div>
                                <datepicker v-model="abaMovimentacoes.intervalo"
                                            :disabled="abaMovimentacoes.preload || !controle.dados.por_periodo"
                                            placeholder="Informe um intervalo de data"
                                            label=""
                                            range
                                            style="margin-top: -19px"
                                            separador=" até " @onselect="atualizaContaCorrente">
                                </datepicker>
                            </div>
                            <div class="col-12 col-md-8">
                                <form action="" @submit.prevent="$refs.paginacao.buscar()">
                                    ...
                                    <div class="input-group">
                                        <input v-model="controle.dados.campoBusca" aria-describedby="" aria-label=""
                                               class="form-control" type="text">
                                        <button v-show="controle.dados.campoBusca!==''" class="btn btn-danger"
                                                type="button"
                    ...
                    <div class="col mt-2 mb-2">
                        <button :disabled="abaMovimentacoes.preload" class="btn btn-sm btn-success" type="button"
                                @click="atualizaContaCorrente">
                            <i class="fas fa-sync"></i> Atualizar
                        </button>
                        <button v-if="pode_insert" :disabled="abaMovimentacoes.preload" class="btn btn-sm btn-primary"
                                data-target="#janelaLancamento" data-toggle="modal"
                                type="button" @click="formNovoLancamento">
                            <i class="fas fa-plus"></i> Novo lançamento
                        </button>
                        <button :class="{
                        'btn-outline-primary':abaMovimentacoes.filtrarPor.length===0,
                        'btn-primary':abaMovimentacoes.filtrarPor.length > 0 }" :disabled="abaMovimentacoes.preload"
                                class="btn btn-sm"
                                data-target="#janelaFiltrarLancamentos" data-toggle="modal"
                                type="button">
                    ...
                    <preload v-show="abaMovimentacoes.preload" class="text-center"
                             label="Buscando lançamentos..."></preload>
                    ...
                    <div v-if="!abaMovimentacoes.preload" class="col-12">
                        <h4 v-if="abaMovimentacoes.lista.length===0" class="text-center">
                            ...
                            <span v-show="controle.dados.campoBusca!==''">
                                com o termo"{{ controle.dados.campoBusca }}"
                            </span>
                        </h4>
                        ...
                        <table v-if="abaMovimentacoes.lista.length > 0 && !abaMovimentacoes.preload" class="tabela">
                            ...
                            <tr>
                                ...
                                <td :class="{'text-primary':lancamentosSaldoTotalAntetior >=0,'text-danger': lancamentosSaldoTotalAntetior < 0}"
                                    class="text-right">
                                    R$ {{ lancamentosSaldoTotalAntetior | dinheiro }}
                                </td>
                                <td :class="{'text-primary':lancamentosSaldoTotalAntetior >=0,'text-danger': lancamentosSaldoTotalAntetior < 0}"
                                    class="text-right">
                                    R$ {{ lancamentosSaldoTotalAntetior | dinheiro }}
                            ...
                            <tr v-for="(lan,index) in abaMovimentacoes.lista" :key="lan.id">
                                ...
                                <td :class="{'text-primary':lan.valor >=0,'text-danger': lan.valor < 0}"
                                    class="text-right">
                                    R$ {{ lan.valor | dinheiro }} {{ lan.operacao }}
                                </td>
                                <td :class="{
                                            'text-primary':lan.saldoAtual >=0,
                                            'text-danger': lan.saldoAtual < 0
                                            }" class="text-right">
                                    <strong v-show="lan.concluido">R$ {{ lan.saldoAtual | dinheiro }}</strong>
                                </td>
                                <td :class="{
                                            'text-primary':lan.saldo >=0,
                                            'text-danger': lan.saldo < 0
                                            }" class="text-right">
                                    <strong v-show="!lan.concluido">R$ {{ lan.saldo | dinheiro }}</strong>
                                </td>
                                <td>
                                    <template v-if="lan.data_pendente !== null">
                                        ...
                                        <template v-if="!lan.concluido">
                                            <br>
                                            <small v-if="lan.diasAtraso < 0">Faltam {{ lan.diasAtraso * -1 }}
                                                dias</small>
                                ...
                                <td>
                                    <template v-if="lan.data_hora_concluido !== null && lan.concluido">
                                        ...
                                        <template v-if="lan.concluido">
                                            <br>
                                            <small v-if="lan.diasAtrasoConcluido < 0">{{ lan.diasAtrasoConcluido * -1 }}
                                                dias antes</small>
                                            <small v-if="lan.diasAtrasoConcluido > 0">{{ lan.diasAtrasoConcluido }} dias
                                                depois</small>
                                        </template>
                                    </template>
                                </td>
                                <td class="text-center">
                                    ...
                                    <div v-if="pode_update || pode_delete" class="dropdown">
                                        ...
                                        <div aria-labelledby="dropdownMenuButton"
                                            ...
                                            <a v-if="!formRealizar.preload && pode_realizar && !lan.concluido && lan.data_pendente!=null"
                                            ...
                                            <a v-if="pode_update" class="dropdown-item text-primary"
                                               data-target="#janelaLancamento"
                                               data-toggle="modal"
                                               href="#" @click="formEditarLancamento(lan)">
                                                <i class="far fa-edit"></i> Editar
                                            </a>
                                            <a v-if="pode_delete" class="dropdown-item text-danger"
                                ...
                                <td class="text-right">
                                    <strong>
                                        <span
                                            ...
                                            R$ {{ lancamentosSaldoTotalAntetior| dinheiro }}</span>
                                    </strong>
                                </td>
                                <td colspan="3"></td>
                            </tr>
                            <tr>
                                ...
                                <td class="text-right">
                                    <strong>
                                        <span class="text-primary">R$ {{ lancamentosTotalReceitas | dinheiro }}</span>
                                    </strong>
                                </td>
                                <td colspan="3"></td>
                            </tr>
                                ...
                                <td class="text-right">
                                    <strong>
                                        <span class="text-danger">
                                            R$ {{ lancamentosTotalDespesas | dinheiro }}
                                        </span>
                                    </strong>
                                </td>
                                <td colspan="3"></td>
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    <strong>
                                        <span
                                            :class="{'text-primary': lancamentosSaldoTotal >=0, 'text-danger': lancamentosSaldoTotal < 0 }">
                                            R$ {{ lancamentosSaldoTotal + lancamentosSaldoTotalAntetior | dinheiro }}
                                        </span>
                                    </strong>
                                </td>
                                <td colspan="3"></td>
                    ...
                    <controle-paginacao ref="paginacao" :dados="controle.dados"
                                        :url="`fluxo-caixa/${id}/atualizaFluxoCaixa`"
                                        class="d-flex justify-content-center"
                                        por-pagina="30"
                                        @carregando="carregando" @carregou="carregou"></controle-paginacao>
                </div>
            </div>
        </div>
    </div>
...
export default {
    ...
    data() {
        return {
            ...
            lancamentosTotalReceitas: 0.00,
            lancamentosTotalDespesas: 0.00,
            lancamentosSaldoTotal: 0.00,
            lancamentosSaldoTotalAntetior: 0.00,
    ...
    mounted() {
        ...
        this.preload = true;
        axios.get(`${URL_ADMIN}/fluxo-caixa/${this.id}`)
            .then((response) => {
    ...
    methods: {
        ...
        selecionarTodosPlanosFiltro() {
            ...
            if (this.abaMovimentacoes.tudoMarcado) { //abaMovimentacoes
        ...
        formEditarLancamento(obj) {
            ...
            axios.get(`${URL_ADMIN}/fluxo-caixa/${this.id}/lancamento/${this.lancamento_id}`)
                .then((response) => {
                    let data = response.data;
        ...
        salvarLancamento() {
            ...
            this.formLancamento.formas.forEach((forma) => {
                ...
                if (this.formLancamento.operacao === 'C' && valor < 0.00) {
                    erro_pagamento_credito = true;
                }
                if (this.formLancamento.operacao === 'D' && valor > 0.00) {
            ...
            this.formLancamento.preload = true;
            if (this.formLancamento.editando) { // se estiver editando
                axios.put(`${URL_ADMIN}/fluxo-caixa/${this.id}/lancamento/${this.lancamento_id}`, this.formLancamento)
                    .then((data) => {
                        this.formLancamento.preload = false;
                ...
                axios.post(`${URL_ADMIN}/fluxo-caixa/${this.id}/lancamento/`, this.formLancamento)
                    .then((data) => {
                        this.formLancamento.preload = false;
                        if (this.formLancamento.editando) {
                            this.formLancamento.atualizado = true;
        ...
        apagarLancamento() {
            ...
            axios.delete(`${URL_ADMIN}/fluxo-caixa/${this.id}/lancamento/${this.lancamento_id}`, this.formApagar)
                .then((response) => {
                    this.formApagar.preload = false;
        ...
        mudarStatus(obj, status) {
            ...
            axios.put(`${URL_ADMIN}/fluxo-caixa/${this.id}/lancamento/${obj.id}/mudarStatus`, {status: status})
                .then((response) => {
