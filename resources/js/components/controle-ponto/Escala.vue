<template>
    <div>
        <button type="button" class="btn btn-primary mt-2" @click="addJornada" v-if="botao_add_jornada">
            <i class="far fa-calendar-plus"></i> Adicionar jornada
        </button>
        <template v-for="(jornada, index) in jornadas" :key="jornada.id || index">
            <div
                class="card text-white mt-3"
                :style="{ border: topo_card ? '1px solid rgb(204, 204, 204)' : '' }"
                :class="{ 'border-primary': topo_card }"
            ...
            >
                            ...
                            <div class="form-row" v-if="exibir_controles_jornada">
                                ...
                                <div class="col-md-2 d-flex flex-row-reverse align-content-end flex-wrap">
                                    ...
                                    <input
                                        ...
                                        class="form-control"
                                        placeholder="Dias"
                                        v-model="jornada.repetir"
                                        @change="validaCampoRepetir(jornada)"
                                        @input="validaCampoRepetir(jornada)"
                                ...
                                <div class="col-md-3 d-flex flex-row-reverse align-content-end flex-wrap">
                                    ...
                                    <select id="inputState" class="form-control" v-model="jornada.ocorrencia_id">
                                        <option v-for="ocorrencia in ocorrencias" :key="ocorrencia.id" :value="ocorrencia.id">
                                ...
                                <div class="col-md-5 d-flex align-content-end flex-wrap">
                                    <button
                                        ...
                                        @click="addPeriodo(jornada)"
                                    >
                                        Adicionar período <i class="far fa-clock"></i>
                                    </button>
                                </div>
                ...
                <div class="card-body" v-if="jornada.ocorrenciaSelecionada.trabalhado">
                    <div class="row">
                        <div class="col-12 col-md-6" v-for="(periodo, index) in jornada.periodos" :key="periodo.id || index">
                            <div class="card" style="border: 1px solid rgb(219, 220, 220)">
                                <div class="card-header bg-light text-black-50" style="border: 1px solid rgb(219, 220, 220)">
                                    <div class="row">
                                        <div class="col-11">PERÍODO TRABALHADO {{ index + 1 }}</div>
                                        <div class="col-1" v-if="botao_remover_periodo">
                                            <button
                                                v-if="!periodo.autenticacao_entrada"
                                                type="button"
                                                class="btn btn-light"
                                                @click="removerPeriodo(jornada.periodos, index)"
                                            >
                                        ...
                                        <div class="col-12 col-md-6 col-xl-3">
                                            <div class="input-group mb-2">
                                                ...
                                                <input
                                                    ...
                                                    class="form-control"
                                                    placeholder="00:00"
                                                    :value="periodo.horas_trabalhadas"
                                                    :disabled="true"
                                                />
                                            </div>
                ...
                <div class="row">
                    <div class="col-6">
                        <button
                            v-if="!exibir_controles_jornada && botao_add_periodo"
                            type="button"
                            class="btn btn-light btn-lg btn-block"
                            @click="addPeriodo(jornada, 'inicio')"
                        >
                            <i class="fas fa-sign-out-alt"></i> Adicionar período ao início
                        </button>
                    </div>
                    <div class="col-6">
                        <button
                            ...
                            type="button"
                            class="btn btn-light btn-lg btn-block"
                            @click="addPeriodo(jornada, 'fim')"
                        >
                            Adicionar período ao fim <i class="fas fa-sign-in-alt fa-flip-horizontal"></i>
                        </button>
                    </div>
                </div>
            </div>
        ...
        <div class="alert alert-info" role="alert" v-if="jornadas.length > 0 && info">
            ...
            <p v-else>
                Após a jornada {{ jornadas[jornadas.length - 1].fim }} ({{ jornadas[jornadas.length - 1].fimDia }}) a escala irá circular e a próxima será a
                jornada 1, iniciando {{ jornadas[jornadas.length - 1].momentInicio.format('dddd') }} e repetindo o ciclo.
            </p>
        </div>
    </div>
</template>
            ...
            jornadas.forEach((jornada, indexJornada) => {
                jornada.periodos.forEach((periodo, indexPeriodo) => {
                    ...
                    if (periodo.entrada.length < 5 || periodo.saida.length < 5) {
                        ...
                        //hora de descanço
                        if (indexPeriodo > 0) {
                            // se for a segunda em diantes, sempre pegar a anterior.
                            ...
                            if (indexJornada > 0) {
                                ...
                                if (jornadaAnterior) {
                                    ...
                                    if (periodo.entrada.length < 5 || ultimoPeriodoJornadaAnterior.saida.length < 5) {
                                        ...
                                        let inicio = moment(
                                            `${jornadaAnterior.momentFim.format('DD/MM/YYYY')} ${ultimoPeriodoJornadaAnterior.saida}`,
                                            'DD/MM/YYYY HH:mm'
                                        )
                                ...
                                if (jornadaAnterior) {
                                    let ultimoPeriodoJornadaAnterior =
                                        jornadaAnterior.periodos.length >= 1 ? jornadaAnterior.periodos[jornadaAnterior.periodos.length - 1] : null
