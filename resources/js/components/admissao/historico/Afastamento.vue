<template>
    <div>
        ...
        <div v-if="!preload" :id="`form_${hash}`">
            <button class="btn btn-sm mr-1 btn-primary mb-3" @click="addLIMedida"><i class="fa fa-plus"></i> Adicionar Afastamento</button>

            <template v-if="form.afastamento.length > 0">
                <fieldset class="mb-2" v-for="(obj, index) in form.afastamento" :key="obj.id || index">
                    <legend>#{{ index + 1 }}</legend>
                    <div class="row">
                        <div class="col-md-3">
                            <date-picker label="Período" range v-model="obj.periodo" formsm :disabled="!obj.novo"></date-picker>
                        </div>
                        ...
                        <div class="col-md-12">
                            ...
                            <textarea type="text" rows="3" v-model="obj.observacao" :disabled="!obj.novo" class="form-control form-control-sm"></textarea>
                        </div>
                        ...
                        <div class="col-12 mt-3" v-show="obj.novo">
                            ...
                            <button class="btn btn-sm mr-1 btn-primary" v-if="obj.novo" @click="salvar"><i class="fa fa-save"></i> Salvar</button>

                            <!--                        <button class="btn btn-sm mr-1 btn-primary mt" @click="addLIMedida" v-show="index >=1">-->
...
export default {
    ...
    methods: {
        ...
        salvar() {
            ...
            if (this.form.afastamento[0].id) {
                axios
                    .put(`${URL_ADMIN}/historico/afastamento-historico/${this.feedback_id}`, this.form)
                    .then((response) => {
                ...
                axios
                    .post(`${URL_ADMIN}/historico/afastamento-historico/${this.feedback_id}`, this.form)
                    .then((response) => {
        ...
        async atualizar() {
            ...
            try {
                const res = await axios.get(`${URL_ADMIN}/historico/afastamento-historico/${this.feedback_id}`)
                const data = res.data
