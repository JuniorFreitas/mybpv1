        ...
        <div v-if="!preload" :id="`form_${hash}`">
            ...
            <div class="table-responsive" v-else>
                <span class="small text-right mb-3">
                    Legenda:
                    <i class="fas fa-circle text-warning ml-2"></i> Aguardando <i class="fas fa-circle text-success ml-2"></i> Aprovado pelo Gestor
                ...
                <table class="table table-bordered table-hover table-condensed mt-3" style="font-size: 0.85em">
                    ...
                    <tbody>
                        <tr
                            v-for="item in listaFeriasDados"
                            :key="item.id"
                            :class="
                                !item
                                    ? 'table-warning'
                        ...
                        >
                            ...
                            <td class="text-center" v-else>AGUARDANDO APROVAÇÃO</td>
                        </tr>
                    </tbody>
                </table>
            </div>
...
export default {
    ...
    methods: {
        ...
        async atualizar() {
            this.preload = true
            try {
                const res = await axios.get(`${URL_ADMIN}/historico/ferias/${this.curriculo_id}`)
                const data = res.data
                this.hoje = data.hoje
