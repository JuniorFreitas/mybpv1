    ...
    <div class="container-fluid">
        <div id="componente">
                ...
                <form class="row" @submit.prevent="carregarConfigs()">
                    <div class="col-12 col-md-4">
                        <div class="form-group">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar por gestor ou tipo"
                                autocomplete="off"
            ...
            <div class="alert alert-info border-0 py-2" role="alert">
                ...
                <strong>Importante:</strong> Quando ativo para um processo, o gestor definido aqui
                <strong>substitui inteiramente</strong> o fluxo padrão de aprovação (ex.: gestor de origem/destino
                na Transferência) — ele é o único aprovador dessa etapa e o único notificado.
            </div>
                    ...
                    <div class="card-header-row">
                        <div class="card-left">
                            ...
                            <div class="colaborador-principal">
                                ...
                                <strong>{{ getNomeUsuario(config.gestor_aprovacao_id) }}</strong>
                            </div>
                        ...
                        <div class="card-right">
                            <bt-ativo
                                :rota="`administracao/gestor-aprovacao-config/${config.id}/ativa-desativa`"
                                :model="config"
                    ...
                    <div class="card-details-row card-fluxo">
                        <div class="detail-item detail-fluxo">
                            ...
                            <span class="detail-value">
                                <span class="fluxo-texto fluxo-extra">{{ getNomeUsuario(config.gestor_aprovacao_id) }} (único aprovador)</span>
        ...
        <modal
            ref="modalConfig"
            id="modalConfigGestor"
            :fechar="!salvando"
            :mostrar-botao-fechar-no-rodape="false"
            size="lg"
            :titulo="(modoEdicao ? 'Editar' : 'Nova') + ' Configuração de Gestor Aprovação'"
        >
            <template #conteudo>
                <form @submit.prevent="salvar">
                    <div class="mb-3">
                        ...
                        <small class="form-text text-muted"> Define qual processo terá o gestor aprovação único </small>
                    </div>

                    <div class="mb-3">
                        <label class="form-label"> Gestor Aprovação <span class="text-danger">*</span> </label>
                        <div class="alert alert-info">Este usuário substitui o fluxo de aprovação padrão para o processo selecionado</div>

                        <div class="form-group" v-if="!form.gestor_selecionado">
                        ...
                        <div class="table-responsive" v-if="form.gestor_selecionado">
                            <table class="table table-bordered table-hover table-condensed bg-white">
                                ...
                                <tbody>
                                    <tr>
                                        <td class="text-center">{{ form.gestor_selecionado.nome }}</td>
                                        <td class="text-center">{{ form.gestor_selecionado.login }}</td>
                                        <td class="text-center">
                                            <a href="javascript://" class="btn btn-sm mr-1 btn-danger" @click.prevent="removerGestor">
                    ...
                    <div class="mb-3">
                        <div class="form-check form-switch">
                            <input type="checkbox" class="form-check-input" id="switchAtivoGestor" v-model="form.ativo" />
                            <label class="form-check-label" for="switchAtivoGestor"> Ativo </label>
                        </div>
                        <small class="form-text text-muted"> Apenas uma configuração por tipo de processo pode existir por empresa </small>
                    </div>

                    <div class="alert alert-warning border-0" role="alert">
                        <i class="fa fa-info-circle me-2"></i>
                        <strong>Lembre-se:</strong> gestor de origem e destino <strong>não</strong> participam nem são notificados
                        quando esta configuração está ativa. O fluxo segue direto pra
                        <strong>{{ form.gestor_selecionado ? form.gestor_selecionado.nome : 'este gestor' }} → RH (final)</strong>.
                    </div>
                </form>
            </template>
            <template #rodape>
...
export default {
    ...
    computed: {
        configsFiltrados() {
            ...
            return this.configs.filter((c) => {
                const nomeGestor = this.getNomeUsuario(c.gestor_aprovacao_id).toLowerCase()
                const tipo = (this.tiposProcesso[c.tipo_processo] || c.tipo_processo || '').toLowerCase()
                return nomeGestor.includes(q) || tipo.includes(q)
            })
        }
    },
    ...
    methods: {
        async carregarTiposProcesso() {
            try {
                const response = await axios.get('/g/administracao/gestor-aprovacao-config/tipos-processo')
        ...
        async carregarUsuarios() {
            ...
            try {
                const response = await axios.get('/g/administracao/gestor-aprovacao-config/listar-usuarios')
        ...
        async carregarConfigs() {
            this.loading = true
            try {
                const response = await axios.get('/g/administracao/gestor-aprovacao-config/listar')
        ...
        getNomeUsuario(userId) {
            ...
            return usuario ? usuario.nome : `ID ${userId}`
        },
        ...
        abrirModal(config = null) {
            if (config) {
                ...
                const usuario = this.todosUsuarios.find((u) => u.id === config.gestor_aprovacao_id)

                this.form = {
                    id: config.id,
                    tipo_processo: config.tipo_processo,
                    gestor_selecionado: usuario ? { id: usuario.id, nome: usuario.nome, login: usuario.login } : null,
                    ativo: config.ativo
                }
            } else {
        ...
        removerGestor() {
            this.form.gestor_selecionado = null
        },

        async salvar() {
            if (!this.form.tipo_processo || !this.form.gestor_selecionado) {
            ...
            try {
                const dados = {
                    ...
                    gestor_aprovacao_id: this.form.gestor_selecionado.id,
                    ativo: this.form.ativo
                }

                let response
                if (this.modoEdicao) {
                    response = await axios.put(`/g/administracao/gestor-aprovacao-config/${this.form.id}`, dados)
                } else {
                    response = await axios.post('/g/administracao/gestor-aprovacao-config', dados)
                }
        ...
        async deletar(id) {
            ...
            try {
                await axios.delete(`/g/administracao/gestor-aprovacao-config/${id}`)
