                ...
                <div class="row align-items-end">
                    <div class="col-12 col-md-6 col-lg-4 mb-2 mb-md-0">
                        <date-range-filter
                            ...
                            :start-date.sync="controle.dados.data_inicio"
                            :end-date.sync="controle.dados.data_fim"
                    ...
                    <div class="col-12 col-md-6 col-lg-4 d-flex align-items-center flex-wrap">
                        ...
                        <button type="button" class="btn btn-sm btn-secondary mb-1" @click="limparFiltros">
                    ...
                    <div class="card-right">
                        <div class="dropdown show">
                            ...
                            <div class="dropdown-menu dropdown-menu-custom dropdown-menu-right"
                                 :aria-labelledby="'dropdownDoc_' + item.id">
        ...
        <modal :id="'modalDetalheDoc_' + hash" titulo="Documento para assinatura" :size="75">
            <template slot="conteudo">
                <div v-if="detalhe" class="container-fluid">
                    <p><strong>ID:</strong> {{ detalhe.id }} &nbsp;|&nbsp; <strong>Tipo:</strong> {{ labelTipo(detalhe.tipo_documento) }} &nbsp;|&nbsp; <strong>Status:</strong> <span class="badge" :class="badgeStatus(detalhe.status)">{{ labelStatus(detalhe.status) }}</span></p>
                    <p><strong>Solicitante:</strong> {{ (detalhe.solicitante && detalhe.solicitante.nome) || '—' }} &nbsp;|&nbsp; <strong>Criado em:</strong> {{ formatarData(detalhe.created_at) }}</p>
                    <p v-if="detalhe.status === 'em_assinatura'" class="text-warning mb-2"><i class="fas fa-hourglass-half"></i> Documento pendente de assinatura</p>
                    <p v-else-if="podeBaixarAssinado(detalhe)" class="mb-2">
                        <a :href="urlDownloadAssinado(detalhe.id)" target="_blank" rel="noopener" class="btn btn-sm btn-success"><i class="fa fa-download"></i> Baixar documento assinado</a>
                    </p>
            ...
            <template slot="rodape">
                <button v-if="detalhe && podeCancelar(detalhe)" type="button" class="btn btn-sm btn-danger mr-1" @click="cancelarNoModal">Cancelar documento</button>
                <button v-if="detalhe && podeReenviar(detalhe)" type="button" class="btn btn-sm btn-warning mr-1" @click="reenviarNoModal">Reenviar e-mail</button>
            </template>
        </modal>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        abrirDetalhe(id) {
            this.detalhe = null;
            axios.get(`${URL_ADMIN}/administracao/documento-assinatura/${id}`).then(res => {
        ...
        executarCancelar(id) {
            axios.post(`${URL_ADMIN}/administracao/documento-assinatura/${id}/cancelar`).then(res => {
        ...
        reenviarEmail(item) {
            ...
            axios.post(`${URL_ADMIN}/administracao/documento-assinatura/${item.id}/reenviar-email`).then(res => {
...
.documento-assinatura-grid .solicitacao-card.card-status-em_assinatura { border-left-color: #ffc107; }
.documento-assinatura-grid .solicitacao-card.card-status-concluido { border-left-color: #28a745; }
.documento-assinatura-grid .solicitacao-card.card-status-cancelado { border-left-color: #dc3545; }
.documento-assinatura-grid .solicitacao-card.card-status-expirado { border-left-color: #6c757d; }
.documento-assinatura-grid .solicitacao-card.card-status-rascunho { border-left-color: #6c757d; }
.documento-assinatura-grid .solicitacao-card.card-status-enviado { border-left-color: #17a2b8; }
...
.documento-assinatura-grid .colaborador-principal i { color: #174257; flex-shrink: 0; }
...
.documento-assinatura-grid .status-em_assinatura { background: #ffc107; color: #212529; }
.documento-assinatura-grid .status-concluido { background: #28a745; color: white; }
.documento-assinatura-grid .status-cancelado { background: #dc3545; color: white; }
.documento-assinatura-grid .status-expirado { background: #6c757d; color: white; }
.documento-assinatura-grid .status-rascunho { background: #6c757d; color: white; }
.documento-assinatura-grid .status-enviado { background: #17a2b8; color: white; }
...
.documento-assinatura-grid .dropdown-menu-custom .dropdown-item i { width: 1.25rem; text-align: center; }
.documento-assinatura-grid .dropdown-menu-custom .dropdown-item:hover { background-color: #f8f9fa; color: #174257; }
...
.documento-assinatura-grid .card-details-row.card-details-main { padding-top: 0.75rem; }
...
.documento-assinatura-grid .signatarios-legend i { margin-right: 0.35rem; color: #174257; }
...
.documento-assinatura-grid .signatario-status.assinado { color: #28a745; }
.documento-assinatura-grid .signatario-status.pendente { color: #6c757d; }
.documento-assinatura-grid .signatario-status.recusado { color: #dc3545; }
.documento-assinatura-grid .signatario-status.expirado { color: #6c757d; }
...
.documento-assinatura-grid .detail-item i:first-child { flex-shrink: 0; font-size: 0.875rem; color: #6c757d; }
.documento-assinatura-grid .detail-label { font-weight: 500; color: #6c757d; white-space: nowrap; }
.documento-assinatura-grid .detail-value { color: #212529; font-weight: 400; }
.documento-assinatura-grid .detail-value-empty { color: #adb5bd; font-style: italic; }
.documento-assinatura-grid .detail-value-expirado { color: #dc3545; font-weight: 600; }
...
@media (max-width: 768px) {
    ...
    .documento-assinatura-grid .card-right { width: 100%; justify-content: flex-end; }
    .documento-assinatura-grid .card-details-row { flex-direction: column; gap: 0.5rem; }
}
