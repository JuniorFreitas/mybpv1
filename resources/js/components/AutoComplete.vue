...
<template>
    <div class="autocomplete">
        <input
            ...
            :class="[leitura ? 'form-control-plaintext' : 'form-control', valido ? 'is-valid' : '', formsm ? 'form-control-sm' : '']"
            :readonly="leitura"
            :disabled="disabled"
            type="text"
            :id="id"
        ...
        <ul id="autocomplete-results" v-show="isOpen" class="autocomplete-results">
            <li class="loading" v-if="isLoading">
                <preload label="Buscando ..."></preload>
            </li>
            <li
                v-else
            ...
            if (this.isAsync) {
                ...
                this.intervalo = setTimeout(() => {
                    ...
                    if (this.metodo === 'get') {
                        ...
                        axios
                            .post(`${URL_ADMIN}/${this.caminho}`, this.dados)
                            .then(({ data }) => {
                                this.isLoading = false
                                this.results = data
                                if (data.length > 0) {
