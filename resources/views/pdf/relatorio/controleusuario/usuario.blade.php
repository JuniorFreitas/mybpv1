@extends('layouts.pdf')
@section('title','RELATÓRIO - '.$request->autocomplete_label_usuario_modal)
@section('empresa')
    @include('layouts.cabecalioEmpresa')
@endsection
@section('conteudo')
    <h5 class="text-center" style="margin-top: 30px">RELATÓRIO USUARIO - ({{$request->autocomplete_label_usuario_modal}}) periodo {{(new \MasterTag\DataHora($dataInicio))->dataCompleta()}} à {{(new \MasterTag\DataHora($dataFim))->dataCompleta()}}</h5>
