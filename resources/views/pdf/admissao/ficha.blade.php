@extends('layouts.pdf_filial')
@section('title','Relatorio de ponto')
@section('conteudo')
    <div style="margin-left: 9px">
        @include('layouts.cabecalioFilialEmpresaJob')

        <h2 class="text-center">FICHA DE ADMISSÃO</h2>


{{--        <table width="97%" style="border: 1px solid #666666; padding: 8px 17px 15px">--}}
{{--            <tr>--}}
{{--                <td>--}}
{{--                    <p style="line-height: 17pt; font-size: 8.5pt; text-transform: uppercase">--}}

{{--                        Nome: <strong>{{ $dados['dados_colaborador']->Curriculo->nome }}</strong>--}}
{{--                        | Data de Nascimento: <strong>{{ $dados['dados_colaborador']->Curriculo->nascimento }}--}}
{{--                            ({{ $dados['dados_colaborador']->Curriculo->idade }}--}}
{{--                            anos)</strong>--}}
{{--                        | PCD: <strong>{{ $dados['dados_colaborador']->Curriculo->PCD ? 'Sim' : 'Não' }}</strong>--}}
{{--                        | CNH:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRh->cnh ? $dados['dados_colaborador']->ParecerRh->cnh_tipo : 'Não possui' }}</strong>--}}
{{--                        | CPF: <strong>{{ $dados['dados_colaborador']->Curriculo->cpf ?? 'Não informado' }}</strong>--}}
{{--                        | RG: <strong>{{ $dados['dados_colaborador']->Curriculo->rg ?? 'Não informado' }}</strong>--}}
{{--                        | RG Data Emissão:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->Curriculo->rg_data_emissao ?? 'Não informado' }}</strong>--}}
{{--                        | Naturalidade:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->Curriculo->naturalidade ?? 'Não informado' }}</strong>--}}
{{--                        | Sexo: <strong>{{ $dados['dados_colaborador']->Curriculo->sexo ?? 'Não informado' }}</strong>--}}
{{--                        | Estado Civil:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->Curriculo->estado_civil ?? 'Não informado' }}</strong>--}}
{{--                        | Mãe:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->Curriculo->filiacao_mae ?? 'Não informado' }}</strong>--}}
{{--                        | Pai:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->Curriculo->filiacao_pai ?? 'Não informado' }}</strong>--}}
{{--                        | Calça: <strong>{{ $dados['dados_colaborador']->ParecerRh->calca }}</strong>--}}
{{--                        | Bota: <strong>{{ $dados['dados_colaborador']->ParecerRh->bota }}</strong>--}}
{{--                        | C. Meia: <strong>{{ $dados['dados_colaborador']->ParecerRh->camisa_meia }}</strong>--}}
{{--                        | C. Proteção: <strong>{{ $dados['dados_colaborador']->ParecerRh->camisa_protecao }}</strong>--}}
{{--                        | Vaga:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->VagaAberta->VagaSelecionada->nome  . ' - ' . $dados['dados_colaborador']->VagaAberta->Municipio->uf}}</strong>--}}
{{--                        | Contato:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->TelPrincipal ? $dados['dados_colaborador']->TelPrincipal->numero: 'Não informado' }}</strong>--}}
{{--                        | E-mail: <span--}}
{{--                            style="text-transform: lowercase"><strong>{{ $dados['dados_colaborador']->Curriculo->email }}</strong></span>--}}
{{--                        | Ex Funcionário:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRh->ex_funcionario == true ? 'Sim' : 'Não' }}</strong>--}}
{{--                        | Disponibilidade para turnos 6X2:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRh->turnos_seis_por_dois ? 'Sim' : 'Não' }}</strong>--}}
{{--                        @if ($dados['dados_colaborador']->ParecerRh->indicado)--}}
{{--                            | Indicado por: <strong>{{ $dados['dados_colaborador']->ParecerRh->indicado_por}}</strong>--}}

{{--                        @endif--}}
{{--                        | Indicado para qual área:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTecnica->indicado_area ?? 'Não informado' }}</strong>--}}
{{--                        | Endereço: <strong>{{ $dados['dados_colaborador']->Curriculo->endereco_completo }}</strong>--}}
{{--                        | Bairro Rota:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->bairro_rota : 'Não Informado' }}</strong>--}}
{{--                        | Ponto Referência Rota:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->ponto_referencia_rota : 'Não Informado' }}</strong>--}}
{{--                        | Ponto Referência Bairro:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->ponto_referencia_residencia : 'Não Informado' }}</strong>--}}
{{--                        | Teste aplicado:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->parecer_final_teste ?: 'Não Informado' }}</strong>--}}
{{--                        | Resultado Teste Prático:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->parecer_final_teste ?: 'Não Informado' }}</strong>--}}
{{--                        | Rigger:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->experiencia_cargas_rigger ?: 'Não Informado' }}</strong>--}}
{{--                        | Plataforma Movél:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->opera_plat_movel ?: 'Não Informado' }}</strong>--}}
{{--                        | Ponte Rolante:--}}
{{--                        <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->opera_plat_ponte ?: 'Não Informado' }}</strong>--}}
{{--                    </p>--}}
{{--                </td>--}}
{{--                <td width="4cm" style="border-left: 1px solid #666666;" align="center">--}}
{{--                    @if (count($dados['dados_colaborador']->Curriculo->FotoTres)>0)--}}
{{--                        <img--}}
{{--                            src="{{env('AWS_URL')}}/arquivos/{{  $dados['dados_colaborador']->Curriculo->FotoTres[0]->disco }}/{{ $dados['dados_colaborador']->Curriculo->FotoTres[0]->thumb }}"--}}
{{--                            style="height: 4cm; ">--}}

{{--                    @else--}}
{{--                        <img--}}
{{--                            src="data:image/jpeg;base64,"--}}
{{--                            style="height: 4cm; ">--}}

{{--                    @endif--}}
{{--                </td>--}}
{{--            </tr>--}}
{{--        </table>--}}

        <table class="table_ficha">
            <tr>
                <th colspan="4">DADOS PESSOAIS</th>
                <th>Foto 3x4</th>
            </tr>
            <tr>
                <td style="width: 187px">
                    Nome Completo:<br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->nome }}</strong>
                </td>
                <td>
                    Data de Nascimento: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->nascimento }}
                        ({{ $dados['dados_colaborador']->Curriculo->idade }} anos)</strong>
                </td>
                <td>
                    PCD: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->PCD ? 'Sim' : 'Não' }}</strong>
                </td>
                <td>
                    CNH: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRh->cnh ? $dados['dados_colaborador']->ParecerRh->cnh_tipo : 'Não possui' }}</strong>
                </td>
                <td rowspan="4" style="width: 3cm; height: 4cm">
                    @if (count($dados['dados_colaborador']->Curriculo->FotoTres)>0)
                        <img
                            src="{{env('AWS_URL')}}/arquivos/{{  $dados['dados_colaborador']->Curriculo->FotoTres[0]->disco }}/{{ $dados['dados_colaborador']->Curriculo->FotoTres[0]->thumb }}"
                            style="height: 4cm; ">
                    @else
                        <img
                            src="data:image/jpeg;base64,"
                            style="height: 4cm; ">
                    @endif
                </td>
            </tr>
            <tr>
                <td>CPF: <br> <strong>{{ $dados['dados_colaborador']->Curriculo->cpf ?? 'Não informado' }}</strong></td>
                <td>RG: <br><strong>{{ $dados['dados_colaborador']->Curriculo->rg ?? 'Não informado' }}
                        ({{$dados['dados_colaborador']->Curriculo->orgao_expeditor}})</strong></td>
                <td>RG Emissão: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->rg_data_emissao ?? 'Não informado' }}</strong>
                </td>
                <td>
                    Naturalidade: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->naturalidade ?? 'Não informado' }}</strong>
                </td>
            </tr>
            <tr>
                <td>Sexo: <br> <strong>{{ $dados['dados_colaborador']->Curriculo->sexo ?? 'Não informado' }}</strong>
                </td>
                <td>Estado Civil: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->estado_civil ?? 'Não informado' }}</strong></td>
                <td>Ex Funcionário: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRh->ex_funcionario == true ? 'Sim' : 'Não' }}</strong>
                </td>
                <td>
                    E-mail: <br><span
                        style="text-transform: lowercase"><strong>{{ $dados['dados_colaborador']->Curriculo->email }}</strong></span>
                </td>
            </tr>
            <tr>
                <td>Mãe: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->filiacao_mae ?? 'Não informado' }}</strong></td>
                <td colspan="2">Pai: <br>
                    <strong>{{ $dados['dados_colaborador']->Curriculo->filiacao_pai ?? 'Não informado' }}</strong></td>
                <td>Contato: <br>
                    <strong>{{ \App\Models\Curriculo::getTelPrincipal($dados['dados_colaborador']->curriculo_id) ?? 'Não informado' }}</strong>
                </td>
            </tr>
            <tr>
                <th colspan="4">ENDEREÇO</th>
                <th></th>
            </tr>
            <tr>
                <td colspan="5" style="text-align: center">
                    <strong>{{ $dados['dados_colaborador']->Curriculo->endereco_completo }}</strong></td>
            </tr>

            @if(!empty($dados['dados_colaborador']->Curriculo->Dependentes))
                <tr>
                    <th colspan="4">DEPENDENTES</th>
                    <th></th>
                </tr>
                @foreach($dados['dados_colaborador']->Curriculo->Dependentes as $dependentes)
                    <tr>
                        <td>
                            Tipo: <br>
                            <strong>{{ $dependentes->tipo == 'outro' ? $dependentes->outro_tipo : \App\Models\UsuarioDependente::TIPOS_DEPENDENTES[$dependentes->tipo] }}</strong>
                        </td>
                        <td colspan="2">
                            Nome: <br>
                            <strong>{{ $dependentes->nome}}</strong>
                        </td>
                        <td>
                            CPF: <br>
                            <strong>{{ $dependentes->cpf ?? 'Não informado'}}</strong>
                        </td>
                        <td>
                            Data de Nascimento: <br> <strong>{{ $dependentes->nascimento ?? 'Não informado'}}</strong>
                        </td>
                    </tr>
                @endforeach
            @endif

            <tr>
                <th colspan="4">DADOS EPI</th>
                <th></th>
            </tr>
            <tr>
                <td>Calça: <br> <strong>{{ $dados['dados_colaborador']->ParecerRh->calca ?? 'Não informado' }}</strong>
                </td>
                <td>C. Meia: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRh->camisa_meia ?? 'Não informado' }}</strong></td>
                <td>Bota: <br> <strong>{{ $dados['dados_colaborador']->ParecerRh->bota ?? 'Não informado'   }}</strong>
                </td>
                <td>C. Proteção: <br> <strong>{{ $dados['dados_colaborador']->ParecerRh->camisa_protecao }}</strong>
                </td>
                <td></td>
            </tr>
            <tr>
                <th colspan="4">OUTRAS INFORMAÇÕES</th>
                <th></th>
            </tr>
            <tr>
                <td>
                    Turnos 6x2: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRh->turnos_seis_por_dois ? 'Sim' : 'Não' }}</strong>
                </td>
                <td>
                    Indicado: <br>
                    {{ $dados['dados_colaborador']->ParecerRh->indicado ? 'Sim' : 'Não'}}
                    @if ($dados['dados_colaborador']->ParecerRh->indicado)
                        <br>Por: <strong>{{ $dados['dados_colaborador']->ParecerRh->indicado_por}}</strong>
                    @endif
                </td>
                <td>Indicado área: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerTecnica->indicado_area ?? 'Não informado' }}</strong>
                </td>
                <td>
                    Bairro Rota: <br> <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->bairro_rota : 'Não Informado' }}</strong>
                </td>
                <td>
                    Ponto Ref. Rota: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->ponto_referencia_rota : 'Não Informado' }}</strong>
                </td>
            </tr>

            <tr>
                <td>Ponto Ref. Bairro: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerRota ? $dados['dados_colaborador']->ParecerRota->ponto_referencia_residencia : 'Não Informado' }}</strong>
                </td>
                <td>
                    Teste aplicado: <br>
                    <strong>{{$dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->parecer_final_teste ?: 'Não Informado' }}</strong>
                </td>
                <td>
                    Resultado Teste Prático: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->parecer_final_teste ?: 'Não Informado' }}</strong>
                </td>
                <td>
                    Rigger: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->experiencia_cargas_rigger ?: 'Não Informado' }}</strong>
                </td>
                <td>
                    Plataforma Movél: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->opera_plat_movel ?: 'Não Informado' }}</strong>
                </td>
            </tr>
            <tr>
                <td>
                    Ponte Rolante: <br>
                    <strong>{{ $dados['dados_colaborador']->ParecerTeste && $dados['dados_colaborador']->ParecerTeste->opera_plat_ponte ?: 'Não Informado' }}</strong>
                </td>
                <td></td>
                <td></td>
                <td></td>
                <td></td>
            </tr>
        </table>

        <table class="table_ficha" style="margin-top: -33px">
            <tr>
                <th colspan="6">DADOS DE ADMISSÃO</th>
            </tr>
            <tr>
                <td>
                    Função: <br> <strong>{{ $dados['dados_colaborador']->Admissao->funcao ?? 'Não informado' }}</strong>
                </td>
                <td>
                    Cargo: <br> <strong>{{ $dados['dados_colaborador']->Admissao->cargo ?? 'Não informado' }}</strong>
                </td>
                <td>
                    Salário R$: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->salario ?? 'Não informado'}}</strong>
                </td>
                <td>
                    Documento: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->documento ?? 'Não informado'}}</strong>
                </td>
                <td>
                    Documento Portaria: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->documento_portaria ?? 'Não informado' }}</strong>
                </td>
                <td>
                    Tipo de admissão: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->tipo_admissao ?? 'Não informado' }}</strong>
                </td>
            </tr>

            <tr>
                <td>Treinamento: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->treinamento ?? 'Não informado' }}</strong></td>
                <td>Tipo de Treinamento: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->tipo_treinamento ?? 'Não informado' }}</strong>
                </td>
                <td>Data Treinamento: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->data_treinamento ?? 'Não informado' }}</strong>
                </td>
                <td>Número Crachá: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->numero_cracha ?? 'Não informado' }}</strong></td>
                <td>Data do ASO: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->UltimoAsoAtivo->data_aso ?? 'Não informado' }}</strong>
                </td>
                <td>Data da Admissão: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->data_admissao ?? 'Não Informado'}}</strong></td>
            </tr>

            <tr>
                <th colspan="6">DOCUMENTOS</th>
            </tr>
            <tr>
                <td>
                    PIS: <br> <strong>{{$dados['dados_colaborador']->Admissao->pis ?? 'Não informado'}}</strong>
                </td>
                <td>
                    CTPS: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_numero ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    CTPS Série: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_serie ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    CTPS UF: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_uf ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    Título de Eleitor: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_numero ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    Título Sessão: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_sessao ?? 'Não Informado'}}</strong>
                </td>
            </tr>

            <tr>
                <td>
                    Título Zona: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_zona ?? 'Não Informado'}}</strong>
                </td>
                <td> Cert. Reservista Nº: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->cert_reservista_num ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    Cert. Reservista Cat: <br>
                    <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes->cert_reservista_categoria ?? 'Não Informado'}}</strong>
                </td>
                <td></td>
                <td></td>
                <td></td>
            </tr>

            <tr>
                <th colspan="6">DADOS BANCÁRIOS</th>
            </tr>

            <tr>
                <td colspan="2">
                    Chave PIX: <br>
                    @if($dados['dados_colaborador']->BancoConta->pix)
                        <strong>{{ $dados['dados_colaborador']->BancoConta->chavepix ?? 'Não Informado'}} -
                            ({{ $dados['dados_colaborador']->BancoConta->tipochavepix ?? 'Não Informado'}})</strong>
                    @else
                        <strong>Não possui chave cadastrada</strong>
                    @endif
                </td>
                <td colspan="2">
                    Banco: <br>
                    <strong>{{ $dados['dados_colaborador']->BancoConta->banco ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    Agência: <br>
                    <strong>{{ $dados['dados_colaborador']->BancoConta->agencia ?? 'Não Informado'}}</strong>
                </td>
                <td>
                    Conta: <br>
                    <strong>{{ $dados['dados_colaborador']->BancoConta->agencia ?? 'Não Informado'}}</strong>
                </td>
            </tr>

        </table>

        {{--    <hr style="border: transparent; background: #666666; height: 1px;margin-bottom: 5px;">--}}

        {{--        <table style="border: 1px solid #666666; padding: 8px 17px 15px" width="97%">--}}
        {{--            <tr>--}}
        {{--                <td>--}}
        {{--                    <p style="line-height: 15pt; font-size: 8.5pt; text-transform: uppercase; text-align: justify">--}}
        {{--                        Função: <strong>{{ $dados['dados_colaborador']->Admissao->funcao ?? 'Não informado' }}</strong>--}}
        {{--                        | Cargo: <strong>{{ $dados['dados_colaborador']->VagaAberta->VagaSelecionada->nome }}</strong>--}}
        {{--                        | Salário R$:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->salario ?? 'Não informado'}}</strong>--}}
        {{--                        | Documento:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->documento ?? 'Não informado'}}</strong>--}}
        {{--                        | Documento Portaria:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->documento_portaria ?? 'Não informado' }}</strong>--}}
        {{--                        | Tipo de admissão:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->tipo_admissao ?? 'Não informado' }}</strong>--}}
        {{--                        | Treinamento:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->treinamento ?? 'Não informado' }}</strong>--}}
        {{--                        | Tipo de Treinamento:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->tipo_treinamento ?? 'Não informado' }}</strong>--}}
        {{--                        | Data Treinamento:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->data_treinamento ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | NR 33: <strong>{{ $dados['dados_colaborador-]>Admissao->nr_trinta_tres ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | Data NR 33: <strong>{{ $dados['dados_colaborador-]>Admissao->data_nr_trinta_tres ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | NR 35: <strong>{{ $dados['dados_colaborador-]>Admissao->nr_trinta_cinco ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | Data NR 35: <strong>{{ $dados['dados_colaborador-]>Admissao->data_nr_trinta_cinco ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | 3260: <strong>{{ $dados['dados_colaborador-]>Admissao->trinta_dois_sessenta ?? 'Não informado' }}</strong>--}}
        {{--                        --}}{{--                    | Data 3260: <strong>{{ $dados['dados_colaborador-]>Admissao->data_trinta_dois_sessenta ?? 'Não informado' }}</strong>--}}
        {{--                        | Número Crachá:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->numero_cracha ?? 'Não informado' }}</strong>--}}
        {{--                        | Data do ASO:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->UltimoAsoAtivo->data_aso ?? 'Não informado' }}</strong>--}}
        {{--                        | PIS: <strong>{{$dados['dados_colaborador']->Admissao->pis ?? 'Não informado'}}</strong>--}}
        {{--                        | CTPS:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_numero :'Não Informado'}}</strong>--}}
        {{--                        | CTPS Série:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_serie :'Não Informado'}}</strong>--}}
        {{--                        | CTPS Data Emissão:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_data_emissao :'Não Informado'}}</strong>--}}
        {{--                        | CTPS UF:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->ctps_uf :'Não Informado'}}</strong>--}}
        {{--                        | Título de Eleitor:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_numero :'Não Informado'}}</strong>--}}
        {{--                        | Título de Eleitor Sessão:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_sessao :'Não Informado'}}</strong>--}}
        {{--                        | Título de Eleitor Zona:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->titulo_eleitor_zona :'Não Informado'}}</strong>--}}
        {{--                        | Certificado Reservista Nº:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->cert_reservista_num :'Não Informado'}}</strong>--}}
        {{--                        | Certificado Reservista Cat:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->DadosAdmissoes ? $dados['dados_colaborador']->Admissao->DadosAdmissoes->cert_reservista_categoria :'Não Informado'}}</strong>--}}
        {{--                        | Banco:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->BancoConta ? $dados['dados_colaborador']->BancoConta->banco :'Não Informado'}}</strong>--}}
        {{--                        | Agência:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->BancoConta ? $dados['dados_colaborador']->BancoConta->agencia :'Não Informado'}}</strong>--}}
        {{--                        | Conta:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->BancoConta ? $dados['dados_colaborador']->BancoConta->conta :'Não Informado'}}</strong>--}}
        {{--                        | Status Carteira de Treinamento e Etiqueta:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->status_carteira_treinamento ?? 'Não Informado' }}</strong>--}}
        {{--                        | Status: <strong>{{ $dados['dados_colaborador']->Admissao->status ?? 'Não Informado'}}</strong>--}}
        {{--                        | Data da Admissão:--}}
        {{--                        <strong>{{ $dados['dados_colaborador']->Admissao->data_admissao ?? 'Não Informado'}}</strong>--}}
        {{--                    </p>--}}
        {{--                </td>--}}
        {{--            </tr>--}}
        {{--        </table>--}}


        {{--        @if(!empty($dados['dados_colaborador']->Curriculo->Dependentes))--}}
        {{--            <br>--}}
        {{--            <h3 style="margin-top: 5px; margin-bottom: 5px;">DEPENDENTES:</h3>--}}
        {{--            --}}{{--    <hr style="border: transparent; background: #666666; height: 1px;margin-bottom: 5px;">--}}

        {{--            <table style="border: 1px solid #666666; padding: 8px 17px 15px" width="97%">--}}
        {{--                <tr>--}}
        {{--                    <td>--}}
        {{--                        @foreach($dados['dados_colaborador']->Curriculo->Dependentes as $dependentes)--}}
        {{--                            <p style="line-height: 15pt; font-size: 8.5pt; text-transform: uppercase; text-align: justify">--}}
        {{--                                Tipo:--}}
        {{--                                <strong>{{ $dependentes->tipo == 'outro' ? $dependentes->outro_tipo : \App\Models\UsuarioDependente::TIPOS_DEPENDENTES[$dependentes->tipo] }}</strong>--}}
        {{--                                | Nome: <strong>{{ $dependentes->nome }}</strong>--}}
        {{--                                | CPF: <strong>{{ $dependentes->cpf ?? 'Não informado'}}</strong>--}}
        {{--                                | Data de Nascimento: <strong>{{ $dependentes->nascimento ?? 'Não informado'}}</strong>--}}
        {{--                            </p>--}}
        {{--                        @endforeach--}}
        {{--                    </td>--}}
        {{--                </tr>--}}
        {{--            </table>--}}
        {{--        @endif--}}


        <div class="f12" style="line-height: 16pt;text-align: center; font-size: 9pt !important; ">
            <br><br>
            <hr style="width: 10cm; margin-top: 5px;  margin-left: 24%; border:none; border-top: 1px solid #333">
            {{$dados['dados_colaborador']->Curriculo->nome}}
        </div>
        <div style="position:fixed; bottom: 35px">
            @include('layouts.rodapePdfFilialJob')
        </div>
    </div>
@stop

@push('style')
    <style>
        .table_ficha {
            border-collapse: collapse;
            width: 98.6%;
            margin-bottom: 20px;
            margin-left: -10px;
        }

        .table_ficha, .table_ficha th, .table_ficha td {
            border: 1px solid black;
            padding: 5px;
        }

        .table_ficha th {
            background-color: #f2f2f2;
        }

        .table_ficha img {
            height: 4cm;
            width: 3cm;
            float: right;
            /*margin-right: 50px;*/
        }
    </style>
@endpush
