...
<table border="0" align="center" cellpadding="0" width="97%" style="width: 100%;padding: 25px;">
    <tr>
        <td style="text-align: justify">
            ...
            <strong>{{ $dados['nome_de'] }}</strong> criou uma nova solicitação de <strong>Valor Extra em Planejamento Movimentação</strong> que está <strong>pendente de aprovação</strong> na etapa de aprovação do gestor.<br>
            @elseif(isset($dados['tipo']) && $dados['tipo'] === 'atualização')
            <strong>{{ $dados['nome_de'] }}</strong> atualizou o status de <strong>Valor Extra em Planejamento Movimentação</strong> na etapa <strong>{{ $dados['etapa'] }}</strong>.<br>
            @if(isset($dados['status_aprovacao']))
            Status: <strong>{{ ucfirst($dados['status_aprovacao']) }}</strong><br>
            @endif
            @else
            Uma solicitação de <strong>Valor Extra em Planejamento Movimentação</strong> está <strong>pendente de aprovação</strong> na etapa <strong>{{ $dados['etapa'] }}</strong>.<br>
