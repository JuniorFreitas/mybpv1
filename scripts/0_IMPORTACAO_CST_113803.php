...
/**
 * Importação CST – empresa 113803 (Congregação de Santa Doroteia do Brasil).
 * Baseado em scripts/0_IMPORTACAO_MAXTEC.php
 *
 * - Planilha: scripts/importacao_cst_2026.xlsx
 * - Município das vagas abertas: São Luís-MA (id 2743)
 * - Cria/reutiliza cargo (vaga), vaga_aberta e centro de custo
 *
 * Uso:
 *   docker compose exec mybpdp php scripts/0_IMPORTACAO_CST_113803.php
 */
...
$dados = $import->dados->map(function ($line) use ($empresa_id, $municipio_id, &$count) {
    ...
    $arrayDados = [
        'curriculo' => [
            ...
            'telefone' => [
                'whatsapp' => $tipoTel,
                'numero' => Sistema::mascaraTelefone((string) ($line['telefone_numero'] ?? '')),
            ],
            'endereco' => [
        ...
        'admissao' => [
            ...
            'data_aso' => parseDataImportacao($line['data_aso'] ?? null)
...
$outputFile = $logDir . '/' . (new \MasterTag\DataHora())->dataInsert() . '_importacao_cst_113803_log.txt';
...
collect($dados)->chunk(200)->each(function ($records) use ($empresa_id, $outputHandle, &$stats) {
    foreach ($records as $record) {
        ...
        if ($validation->fails()) {
            ...
            $msg = 'Erro ao fazer validacao para importação - do CPF: ' . $record['curriculo']['cpf'] . PHP_EOL
        ...
        try {
            ...
            $msg = 'Erro ao importar do CPF: ' . $e->getMessage() . ' - ' . $record['curriculo']['cpf']
