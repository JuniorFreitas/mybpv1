...
class IntermitenteFixoPrevistaFilterApplier
{
    ...
    private function applyCampoStatusAprovacao(Builder $query): void
    {
        ...
        if ($status === 'aprovado_gestor') {
            $query->where('status_aprovacao', IntermitenteFixoPrevista::STATUS_APROVADO)->whereNull('status_aprovacao_rh');
            return;
        }
        if ($status === 'aprovado_rh') {
            $query->where('status_aprovacao_rh', IntermitenteFixoPrevista::STATUS_APROVADO);
            return;
        }
        $query->where(function ($q) {
            $q->where('status_aprovacao', IntermitenteFixoPrevista::STATUS_REPROVADO)
                ->orWhere('status_aprovacao_rh', IntermitenteFixoPrevista::STATUS_REPROVADO);
        });
    }
