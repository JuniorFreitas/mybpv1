...
use App\Models\User;
use App\Services\FeriasPrevista\FeriasPrevistaCsvFileManager;
use App\Services\FeriasPrevista\FeriasPrevistaExportFormatter;
use App\Services\FeriasPrevista\FeriasPrevistaExportQueryBuilder;
...
class JobFeriasPrevistaExportaExcel implements ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            $configExtra = AprovacaoExtraConfig::getConfigAtiva($user->empresa_id, 'ferias');
            $nomeAprovacaoExtra = $configExtra ? $configExtra->nome_aprovacao : null;
            $formatter = new FeriasPrevistaExportFormatter($nomeAprovacaoExtra);
            $fileManager = new FeriasPrevistaCsvFileManager();
            $headers = $formatter->getHeaders();
            $fileManager->createTempFile($headers);
            $query = FeriasPrevistaExportQueryBuilder::forExport($user, $this->filtros);
            ...
            \Log::error('Erro na exportação Férias Prevista CSV: ' . $e->getMessage());
            throw $e;
        }
    }
