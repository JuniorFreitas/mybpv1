...
class JobCorrigePonto implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $data_anteriori = $dia_anterior->dataInsert()." 00:00:00";
        $data_anteriorf = $dia_anterior->dataInsert()." 23:59:59";

        $ponto_eletronico = \DB::select("SELECT *
                                                FROM ponto_eletronicos
                                                WHERE empresa_id = $empresa_id
                                                  AND ocorrencia_id = 4
                                                  AND created_at >= '".$data_anteriori."' AND created_at <= '".$data_anteriorf."'
