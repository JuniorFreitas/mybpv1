...
class DataHora
{
    ...
    // Formatos de data-----------------------------------------------------------
    public function dataInsert() {// cadastrar no banco
        return $this->ano() . '-' . $this->mes() . '-' . $this->dia();
    }

    public function horaInsert() {// cadastrar no banco
        return $this->hora() . ':' . $this->minuto() . ':' . $this->segundo();
    }

    public  function dataHoraInsert() {// cadastrar no banco
    ...
    public function diaExt() {// dia por extenso

        return $this->dia_extM[intval($this->dia())];

    }

    public function diaExtM() {// dia por extenso
    ...
    public function diaSemanaNum() { // retorna dia da semana atual em numero, 1 (segunda) e 7 (domingo)
    ...
    public function setSegundo($segundo) {
        ...
        $this->DateTime->setTime($this->hora, $this->minuto);
    ...
    */
    public function proximoDia($dia, $NAO_ACEITA_IGUAL = TRUE, $FEVEREIRO = TRUE)
    {
        ...
        if ($NAO_ACEITA_IGUAL === FALSE && $dia == $hoje->dia()) {
            ...
            $this->mes = $hoje->mes();
            $this->ano = $hoje->ano();
            $this->DateTime = new DateTime($this->dataInsert());
            return true;
        }
        ...
        if ($dia >= $hoje->dia()) { //se tiver dia para frente de hoje, vai esse mes
            $teste = new DataHora("{$dia}/{$hoje->mes()}/{$hoje->ano()}");
            $this->dia = $teste->dia();
            $this->mes = $teste->mes();
            $this->ano = $teste->ano();
            $this->DateTime = new DateTime($this->dataInsert()); // adicionado para poder atualizar esse objeto interno
            return true;
        } else { // senao é dia do mes posterior
            $teste = new DataHora("{$dia}/{$hoje->mes()}/{$hoje->ano()}");
            $teste->addMes(1);
            $this->dia = $teste->dia();
            $this->mes = $teste->mes();
        ...
        /*while($hoje->dia() != $dia){ // e quando for fevereiro (dia 30)?
            echo "esta :".$hoje->dataCompleta()." vou tentar a proximo<br>";
            if(checkdate($hoje->mes(),intval($hoje->dia())+1,$hoje->ano()) ){
                $hoje->addDia(1);
                echo "mudou para {$hoje->dataCompleta()}<br>";
            }else{
                $hoje->addDia(1);
                break; // sai do while caso a data seguinte não exista
            }
    ...
    public function nomeUnico() {// auxilia em nomes de arquivos
        $this->atual();
    ...
    public function addDia($dias,$atualizaObjetoAtual=true) { // adiciona quantos dias vc quiser passando quantidade de dias.
    ...
    public function subtrairDia($dias) { // subtrai quantos dias vc quiser passando quantidade de dias.
        $dias=intval($dias);
    ...
    public static function diferencaMinutos($hora_menor, $hora_maior) {
        ...
        if ($hora_maior->ano == 'aaaa' || $hora_menor->ano == 'aaaa') {
            $menor = new DateTime($hora_menor->horaInsert());
            $maior = new DateTime($hora_maior->horaInsert());
        } else {
            $menor = new DateTime($hora_menor->dataHoraInsert());
            $maior = new DateTime($hora_maior->dataHoraInsert());
        }
        ...
        return ($VALORES['dia'] * 1440) + ($VALORES['hora'] * 60) + ($VALORES['minuto']); // apenas em minutos
    }

    // quantidade de minutos
    public static function diferencaSegundos($hora_menor, $hora_maior) {
        $VALORES=array();
        ...
        //segundos
        if($invertido){
            $VALORES['segundos']= $dados_difereca->s * -1;
        }else{
            $VALORES['segundos']= $dados_difereca->s;
        }

        return ($VALORES['hora']*3600)+($VALORES['minuto']*60)+($VALORES['segundos']); // apenas em minutos
    }
    ...
    public function atual() { // aplica data e hora atual
    ...
    */
    ...
    public static function dataFormatada($data)
    {
        $dataH = new DataHora($data);
        return $dataH->dataCompleta();
    }
