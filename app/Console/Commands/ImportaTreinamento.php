...
{
    protected $signature = 'mybp:importa-treinamento
    ...
    private function prepararDados(array $dados): array
    {
        ...
        $dados['vencimento_id'] = (int)$dados['vencimento_id'];
    ...
    private function processarRegistroIndividual(array $dados): void
    {
        ...
        if (!$feedback) {
            ...
            $this->registrarResultado($dados, 'FALHA', 'Feedback não encontrado');
            return;
        }
        ...
        try {
            ...
            // Registrar sucesso
            $this->registrarResultado($dados, 'SUCESSO', 'Treinamento processado com sucesso', $treinamentoModel);

        } catch (\Exception $e) {
            $this->registrarResultado($dados, 'FALHA', $e->getMessage());
            throw $e;
        }
    }
    ...
    private function buscarVencimento(int $vencimentoId): ?object
    {
        // Usar cache para vencimentos
        if (isset($this->vencimentoCache[$vencimentoId])) {
            return $this->vencimentoCache[$vencimentoId];
        }

        $vencimento = \DB::table('vencimentos')->where('id', $vencimentoId)->first();

        $this->vencimentoCache[$vencimentoId] = $vencimento;
    ...
    {
        $vencimentoExiste = \DB::table('vencimentos')
            ->where('id', $dados['vencimento_id'])
            ->exists();
        ...
        $pivotData = $this->prepararDadosPivot($treinamentoModel->id, $dados);
    ...
    private function validarVencimentoExiste(int $vencimentoId): void
    {
        $vencimentoExiste = \DB::table('vencimentos')
            ->where('id', $vencimentoId)
            ->exists();

        if (!$vencimentoExiste) {
            throw new \Exception("Vencimento ID {$vencimentoId} não existe na tabela vencimentos");
        }
    }
    ...
    private function sincronizarVencimento(Treinamento $treinamentoModel, array $dados): void
    {
        ...
        if ($vencimentoJaVinculado) {
            $this->atualizarVencimentoExistente($treinamentoModel->id, $dados['vencimento_id'], $novoVencimentoData);
        } else {
            $this->criarNovoVencimento($treinamentoModel->id, $dados['vencimento_id'], $novoVencimentoData);
        }
        ...
        if ($this->option('substituir-vencimentos')) {
            ...
            if (!in_array($dados['vencimento_id'], $this->vencimentosProcessados[$treinamentoModel->id])) {
                $this->vencimentosProcessados[$treinamentoModel->id][] = $dados['vencimento_id'];
            }
        }
    }
    ...
    private function gerarRelatorioCSV(): string
    {
        ...
        $sucessos = count(array_filter($this->resultadosImportacao, fn($r) => $r['status'] === 'SUCESSO'));
        $falhas = $totalRegistros - $sucessos;
