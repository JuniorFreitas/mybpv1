...
{
    public function __construct(private DossieTipoService $dossieTipoService)
    {
    }
    ...
    public function show(Request $request, $feedback)
    {
        ...
        $feedback = FeedbackCurriculo::select('id', 'curriculo_id', 'empresa_id')->whereId($feedback_id)->with(
            array_merge(
                $this->dossieTipoService->listarRelacionamentosEager($empresaId),
        ...
        $tiposDocumento = $this->dossieTipoService->tiposDocumentoAssinatura($empresaId);
        $docs = empty($tiposDocumento)
            ? collect()
            : DocumentoParaAssinatura::withoutGlobalScopes()
        ...
        foreach ($docs as $doc) {
            $tipoModelo = $this->dossieTipoService->tipoDocumentoParaTipoModelo($doc->tipo_documento, $empresaId);
        ...
        return [
            ...
            'secoes' => $this->dossieTipoService->listarSecoes($empresaId),
            'relacionamentos' => $this->dossieTipoService->listarRelacionamentosFormatados($empresaId),
    ...
    public function store(Request $request, $feedback)
    {
        ...
        $empresaId = (int) ($feedback->empresa_id ?? auth()->user()->empresa_id);
        $dados = $request->input();
        ...
        try {
            ...
            foreach ($this->dossieTipoService->listarChaves($empresaId) as $type) {
                $this->handleDocument($type, $dados, $feedback, $empresaId);
            }
    ...
    private function handleDocument($type, $dados, $feedback, ?int $empresaId = null)
    {
        $deleteKey = $type . 'Del';
        $secoesPorChave = collect($this->dossieTipoService->listarSecoes($empresaId))->keyBy('chave');
        $secao = $secoesPorChave->get($type);

        $label = $secao['label'] ?? strtoupper(str_replace('_', ' ', $type));
        $tipo = $secao['tipo'] ?? \Str::studly($type);
        $relacionamento = 'getDocumentoRelacionado' . $tipo;
...
//    DOWNLOAD MODELOS DE IMPRESSÃO COM OS DADOS DOS FUNCIONÁRIOS
    ...
    public function downloadModelo(DossiePdfService $dossiePdfService, $tipo_modelo, $curriculo_id, $feedback_id)
    {
        $colaborador = FeedbackCurriculo::with(['Curriculo.User', 'Admissao'])
            ->whereCurriculoId($curriculo_id)
            ->whereId($feedback_id)
            ->first();

        if (!$colaborador || !$colaborador->Curriculo?->User || !$colaborador->Admissao) {
            abort(404, 'Registro não encontrado para geração do modelo.');
        }
    ...
    {
        $empresaIdAuth = (int) auth()->user()->empresa_id;
        $tiposModeloValidos = $this->dossieTipoService->tiposModeloAssinatura($empresaIdAuth);
        $request->validate([
            'tipo_modelo' => 'required|string|in:' . implode(',', $tiposModeloValidos ?: ['__nenhum__']),
        ...
        if (!$colaborador || $colaborador->empresa_id != $empresaIdAuth) {
