...
# Script de Deploy no ECS - BP Chamados
...
show_banner() {
    echo -e "${BLUE}"
    echo "=========================================="
    echo "    BP Chamados - Deploy no ECS"
    echo "=========================================="
    echo -e "${NC}"
}
...
show_menu() {
    ...
    echo ""
    echo "1) Desenvolvimento (bpchamados-dev)"
    echo "2) QA/Homologação (bpchamados-homol)"
    echo "3) Produção (bpchamados-prod)"
    echo "4) Deploy customizado"
    echo "5) Sair"
    echo ""
    echo -e "${BLUE}Imagem ECR: ${IMAGE_NAME}${NC}"
    echo ""
}
...
deploy_dev() {
    local cluster_name="bpchamados-dev"
    local service_name="bpchamados-dev-service"
...
deploy_qa() {
    ...
    local service_name="bpchamados-homol-service"
...
deploy_prod() {
    ...
    local service_name="bpchamados-prod-service"
    ...
    read -p "Digite a tag imutável da imagem (versão ou sha256:...): " image_tag
    if [ -z "$image_tag" ]; then
        echo -e "${RED}Tag obrigatória em produção. Vazio e 'latest' são proibidos.${NC}"
        return 1
    fi

    if [ "$image_tag" = "latest" ] || [[ "$image_tag" == *:latest ]]; then
        echo -e "${RED}Tag 'latest' é proibida em produção. Use tag versionada ou digest sha256:.${NC}"
        return 1
    fi
